...
def count_subtrees(node):
    subtrees = []
    count_subtrees_helper(node, subtrees)
    return sorted(subtrees)


def count_subtrees_helper(node, subtrees):
    subtree_size = 1
    for child in node.children:
        subtree_size += count_subtrees_helper(child, subtrees)
    subtrees.append(subtree_size)
    return subtree_size
